...
| 左栏 | 中栏 | 右栏 |
|---|---|---|
| 无线 ADB 开关与信息 | 无线调试状态（只读） | 检测更新 / 退出 |
...
- **无线调试状态（只读）**：中栏只显示系统「无线调试」（`adb_wifi_enabled`，Android 11+）的实际值。它与左栏「无线 ADB」操作的是同一个系统开关，因此不再设独立开关；统一由左栏开关走完整流程（写 `adb_wifi_enabled` + 重启 `adbd` + 等动态端口就绪 + 记录开启意愿），避免两个开关状态打架、重复重启 `adbd` 掐断连接
